...
import { FileText, X, Calendar, Download, TrendingUp } from 'lucide-react-native';
...
interface IncomeStatementGeneratorProps {
  providerId: string;
  onGenerated?: () => void;
}
...
export default function IncomeStatementGenerator({
  ...
  const handleGenerate = async () => {
    ...
    try {
      setGenerating(true);
      await generateIncomeStatement(providerId, startDate, endDate);
      setModalVisible(false);
      onGenerated?.();
      Alert.alert('Success', 'Income statement generated successfully');
    } catch (error) {
      console.error('Error generating income statement:', error);
      Alert.alert('Error', 'Failed to generate income statement. Please try again.');
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        <Text style={styles.triggerButtonText}>Generate Income Statement</Text>
      </TouchableOpacity>
            ...
            <View style={styles.header}>
              <View style={styles.headerLeft}>
                ...
                <View>
                  ...
                  <Text style={styles.subtitle}>Generate financial report</Text>
                </View>
              </View>
                  ...
                  >
                    <View style={styles.rangeCardLeft}>
                      <Calendar
                        size={18}
                        color={
                          selectedRange === 'custom' ? colors.primary : colors.textSecondary
                        }
                      />
                      <Text
                        style={[
