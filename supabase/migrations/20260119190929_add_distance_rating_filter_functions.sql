/*
  # Add Distance and Rating Filter Functions

  1. New Functions
    - `find_nearby_service_listings` - Find service listings within a radius with rating filter
    - `find_nearby_jobs` - Find jobs within a radius with rating filter

  2. Purpose
    - Move distance filtering from post-query to database level
    - Improve query performance by filtering at the database layer
    - Support both distance and rating filters natively

  3. Security
    - Functions are PUBLIC (anyone can search)
    - RLS policies on underlying tables still apply
*/

-- ============================================================================
-- FUNCTION: Find nearby service listings with rating filter
-- ============================================================================
...
-- ============================================================================
-- FUNCTION: Find nearby jobs with rating filter
-- ============================================================================
...
BEGIN
  ...
  WHERE
    ...
    -- Price filters (handle both fixed and budget-based pricing)
    AND (
      p_price_min IS NULL
...
-- Add indexes to improve distance calculation performance
CREATE INDEX IF NOT EXISTS idx_service_listings_lat_lng ON service_listings(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
...
-- Add indexes for rating filtering
CREATE INDEX IF NOT EXISTS idx_profiles_rating_average ON profiles(rating_average) WHERE rating_average > 0;
