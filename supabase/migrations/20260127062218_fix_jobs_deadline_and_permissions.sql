/*
  # Fix Jobs RPC Function and Permissions

  ## Issues
  1. Column j.deadline does not exist - need to remove any remaining references
  2. Permission denied for table jobs - RLS policy checking fails

  ## Solution
  1. Ensure get_jobs_cursor_paginated_v2 doesn't reference deadline
  2. Fix RLS policies to handle cases where profile doesn't exist yet
  3. Add better INSERT policy that doesn't fail on profile lookup
*/

-- Drop and recreate the function without deadline reference
...
CREATE POLICY "Customers and hybrids can create jobs"
  ON jobs FOR INSERT
  TO authenticated
  WITH CHECK (
