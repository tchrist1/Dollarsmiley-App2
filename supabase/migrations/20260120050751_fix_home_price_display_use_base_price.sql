/*
  # Fix Home Listings Price Display Regression

  1. Purpose
    - Fix $0 price display on Home listing cards
    - Use correct price fields (base_price) for services
    - Preserve existing pricing logic on detail pages

  2. Root Cause
    - RPC functions select sl.price but many services only have base_price populated
    - Result: NULL prices displayed as $0 on Home cards
    - Detail pages work correctly because they read base_price directly

  3. Fix Strategy
    - Update get_services_cursor_paginated to use COALESCE(sl.price, sl.base_price)
    - Update get_home_feed_snapshot to use COALESCE(sl.price, sl.base_price)
    - Update price filtering to work with both fields
    - Zero data mutations, zero UI changes, zero detail page changes
*/

-- ============================================================================
-- Fix get_services_cursor_paginated function to use correct price field
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    sl.title,
    sl.description,
    -- CRITICAL FIX: Use base_price as primary, fallback to price for backwards compatibility
  ...
  WHERE sl.status = 'Active'
    ...
    -- CRITICAL FIX: Price filtering uses correct field
...
-- ============================================================================
-- Fix get_home_feed_snapshot function to use correct price field
-- ============================================================================
...
BEGIN
  ...
  jobs AS (
    SELECT
      ...
      j.title,
      -- Jobs: Use fixed_price first, then budget_min, then budget_max, fallback to 0
