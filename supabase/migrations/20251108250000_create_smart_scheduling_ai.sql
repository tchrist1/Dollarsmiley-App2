/*
  # Create Smart Scheduling AI System

  ## Overview
  Intelligent scheduling system with AI-powered features including:
  ...
  - `min_booking_notice_hours` (int) - Minimum advance notice
  - `max_bookings_per_day` (int) - Daily capacity
  - `preferred_booking_duration_minutes` (int)
  - `buffer_time_minutes` (int) - Between bookings
  - `travel_time_buffer_minutes` (int) - Travel buffer
  - `auto_accept_within_hours` (int) - Auto-accept window
  - `price_surge_enabled` (boolean) - Dynamic pricing
  - `price_surge_multiplier` (numeric) - 1.0 - 3.0
  - `allow_back_to_back` (boolean) - No buffer bookings
  - `preferred_times` (jsonb) - Time preferences
  - `updated_at` (timestamptz)

  ### 2. `time_slot_suggestions`
  AI-generated time slot recommendations
  - `id` (uuid, primary key)
  ...
  - `reasoning` (text) - Why this slot
  - `factors` (jsonb) - Contributing factors
  - `is_optimal` (boolean) - Best suggestion
  - `price_estimate` (numeric)
  - `travel_time_minutes` (int)
  - `created_at` (timestamptz)
  - `expires_at` (timestamptz)

  ### 3. `scheduling_patterns`
  Learned booking patterns
  - `id` (uuid, primary key)
  - `provider_id` (uuid, references profiles)
  - `pattern_type` (text) - day_of_week, time_of_day, season, weather
  - `pattern_data` (jsonb) - Pattern details
  - `booking_frequency` (numeric) - Bookings per period
  - `avg_duration_minutes` (int)
  - `avg_price` (numeric)
  - `confidence_level` (numeric)
  - `last_updated` (timestamptz)

  ### 4. `workload_predictions`
  Provider workload forecasts
  - `id` (uuid, primary key)
  - `provider_id` (uuid, references profiles)
  - `prediction_date` (date)
  - `predicted_bookings` (int)
  - `predicted_revenue` (numeric)
  - `capacity_utilization` (numeric) - 0-1 (0-100%)
  - `confidence_score` (numeric)
  - `factors` (jsonb)
  - `created_at` (timestamptz)

  ### 5. `travel_time_matrix`
  Pre-calculated travel times between locations
  - `id` (uuid, primary key)
  - `from_location` (text) - Address or coordinates
  - `to_location` (text)
  - `distance_km` (numeric)
  - `travel_time_minutes` (int)
  - `traffic_factor` (numeric) - Time of day multiplier
  - `last_updated` (timestamptz)
  - `UNIQUE(from_location, to_location)`

  ### 6. `scheduling_conflicts`
  Detected conflicts and resolutions
  - `id` (uuid, primary key)
  - `provider_id` (uuid, references profiles)
  - `conflict_type` (text) - double_booking, travel_time, capacity
  - `booking_ids` (uuid[]) - Conflicting bookings
  ...
  - `id` (uuid, primary key)
  - `provider_id` (uuid, references profiles)
  - `service_category_id` (uuid, references categories)
  - `date` (date)
  ...
  - `recommendation` (text) - expand, maintain, reduce
  - `created_at` (timestamptz)

  ## AI Features
  - **Optimal Slot Finding:** Best time based on multiple factors
  - **Workload Balancing:** Even distribution across time
  - **Travel Optimization:** Minimize travel time
  - **Pattern Learning:** Historical booking analysis
  - **Conflict Detection:** Automatic overlap detection
  - **Dynamic Pricing:** Demand-based pricing
  - **Buffer Management:** Smart gap insertion
  - **Capacity Forecasting:** Future availability prediction
  - **Seasonal Adjustment:** Holiday and season awareness
  ...
  - Providers manage own preferences
  - Users see own suggestions
  - Public access to availability

  ## Performance
  - Indexed time slots
  - Cached predictions
  - Pre-calculated travel times
  - Materialized pattern views
*/
