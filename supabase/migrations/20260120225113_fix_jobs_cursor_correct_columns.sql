/*
  # Fix Jobs Cursor Function - Use Correct Column Names

  1. Changes
    - Remove reference to non-existent `deadline` column
    - Use `execution_date_start` instead which exists in the jobs table
  
  2. Details
    - Fixes jobs cursor function to work with actual table structure
    - Enables job listings to show in search results
*/
...
SELECT
  ...
  j.latitude,
  j.longitude,
  j.execution_date_start as deadline,
