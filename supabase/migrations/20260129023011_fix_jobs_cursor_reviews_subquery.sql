/*
  # Fix Jobs Cursor - Remove Invalid reviews.job_id Subquery

  ## Issue
  PostgreSQL error 42703: "column job_id does not exist" in reviews table
  - reviews table doesn't have job_id column (reviews link through bookings)
  - ORDER BY clause has invalid subquery: SELECT COUNT(*) FROM reviews WHERE job_id = dc.id

  ## Solution
  - Remove reviews sort option from Jobs cursor (not applicable for Jobs)
  - Reviews are only for Services (provider ratings)
  - Jobs use bookings count for popularity metric
*/
