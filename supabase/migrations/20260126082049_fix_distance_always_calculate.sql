/*
  # Fix Distance Badge Calculation

  1. Problem
    - distance_miles only calculated when distance filter is active
    - Distance badge missing when no distance filter applied
    - Users see no distance information on listing cards

  2. Solution
    - Always calculate distance_miles when user coordinates provided
    - Separate distance calculation from distance filtering
    - Distance filter (p_distance) only used for WHERE clause filtering
    - Distance display works independently of filtering

  3. Changes
    - Update get_services_cursor_paginated: always calculate distance_miles
    - Update get_jobs_cursor_paginated: always calculate distance_miles
    - Maintain backward compatibility
*/

-- ============================================================================
-- STEP 1: Update get_services_cursor_paginated with correct signature
-- ============================================================================
...
BEGIN
  -- Distance filtering: only when distance radius is provided
  ...
  -- Distance calculation: whenever user coordinates are available
  v_has_user_location := (
    p_user_lat IS NOT NULL AND
  ...
  SELECT
    ...
    -- CRITICAL FIX: Always calculate distance when user location available
    CASE
  ...
  WHERE LOWER(sl.status) = 'active'
    ...
    -- CRITICAL: Only apply distance FILTERING when p_distance is provided
    AND (
...
-- ============================================================================
-- STEP 2: Update get_jobs_cursor_paginated with correct signature
-- ============================================================================
