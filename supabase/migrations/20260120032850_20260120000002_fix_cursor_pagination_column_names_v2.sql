/*
  # Fix Cursor Pagination Column Names

  1. Purpose
    - Fix column name mismatches in cursor pagination functions
    - Align with actual database schema

  2. Changes
    - Drop and recreate functions with correct column names
    - Update service listings: image_url → featured_image_url
    - Update service listings: average_rating → rating_average
    - Update service listings: total_bookings → booking_count
    - Update jobs: budget → COALESCE(fixed_price, budget_min, budget_max)
*/
...
DROP FUNCTION IF EXISTS get_home_feed_snapshot(UUID, INT);
DROP FUNCTION IF EXISTS get_services_cursor_paginated(TIMESTAMPTZ, UUID, INT, UUID, TEXT, DECIMAL, DECIMAL, DECIMAL, TEXT[]);
DROP FUNCTION IF EXISTS get_jobs_cursor_paginated(TIMESTAMPTZ, UUID, INT, UUID, TEXT, DECIMAL, DECIMAL);

-- ============================================================================
-- Recreate get_home_feed_snapshot function with correct columns
-- ============================================================================
...
-- ============================================================================
-- Recreate get_services_cursor_paginated function with correct columns
-- ============================================================================
...
-- ============================================================================
-- Recreate get_jobs_cursor_paginated function with correct columns
-- ============================================================================
