/*
  # Fix Service Listings Price Column Reference

  ## Summary
  Fixes the RPC functions to use `base_price` instead of `price` for service_listings table.
  The service_listings table has a column named `base_price`, not `price`.

  ## Issues Fixed
  - Error: column "sl.price" does not exist in service_listings table
  - Services showing $0 on home screen because price column reference is incorrect
  - Solution: Update all references from sl.price to sl.base_price

  ## Changes
  1. Update get_services_cursor_paginated_v2 to use base_price
  2. Update get_services_cursor_paginated (if exists) to use base_price
*/
