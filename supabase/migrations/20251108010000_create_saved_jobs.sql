/*
  ...
  ### `saved_jobs_stats`
  Tracks statistics for saved jobs
  - Job save counts
  - User activity
...
-- Add save_count to job_postings (denormalized for performance)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'job_postings' AND column_name = 'save_count'
  ) THEN
    ALTER TABLE job_postings ADD COLUMN save_count integer DEFAULT 0;
  END IF;
END $$;
...
BEGIN
  ...
  SELECT
    ...
    jp.budget_max,
    jp.location,
    jp.execution_date_start,
    jp.execution_date_end,
    jp.preferred_time,
    jp.status,
    jp.save_count,
  ...
  LEFT JOIN profiles p ON jp.customer_id = p.id
