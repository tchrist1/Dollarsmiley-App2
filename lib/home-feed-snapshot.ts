...
// ============================================================================
// REALTIME INVALIDATION SYSTEM
// ============================================================================

let realtimeSubscription: any = null;
let lastInvalidationTime = 0;
const INVALIDATION_THROTTLE_MS = 3000; // Throttle invalidations to max once per 3 seconds

/**
 * Subscribe to realtime changes and invalidate snapshots
 * This ensures users see new listings within seconds of creation
 */
export function subscribeToListingChanges(userId: string | null): () => void {
  ...
  // Subscribe to service_listings changes
  const servicesChannel = supabase
    .channel('home_feed_services')
    .on(
      'postgres_changes',
      {
        event: 'INSERT',
      ...
      () => {
        throttledInvalidateSnapshot(userId);
      }
    )
    .on(
      'postgres_changes',
...
/**
 * Throttled invalidation to prevent excessive cache clearing
 */
function throttledInvalidateSnapshot(userId: string | null): void {
  const now = Date.now();

  if (now - lastInvalidationTime < INVALIDATION_THROTTLE_MS) {
    return;
  }
